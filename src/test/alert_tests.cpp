...
BOOST_AUTO_TEST_CASE(PartitionAlertButtercupOnly)
{
    PartitionAlertTestImpl(RegtestActivateButtercup(false), 1500000000, 96 * 2, 12 * 2, 240 * 2);
    RegtestDeactivateButtercup();
}

BOOST_AUTO_TEST_CASE(PartitionAlertButtercupActivates)
{
    // 48 pre buttercup blocks, 96 buttercup blocks will take 48 * 150s + 96 * 75s = 4hrs
    // in the slow case, all of the blocks will be buttercup blocks
    // in the fast case, 96 blocks will be buttercup => 96 * 75s * 2/5 = 2880s spent on buttercup
    // => (14400 - 2880) / (150 * 2/5) = 11520 / 60 = 192 pre buttercup blocks
    PartitionAlertTestImpl(RegtestActivateButtercup(false, 799 - 96), 2000000000, 144, 12 * 2, 192 + 96);
    RegtestDeactivateButtercup();
}
