...
class WalletTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        # Send 26 BTC from 0 to 2 using sendtoaddress call.
        # Second transaction will be child of first, and will require a fee
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 13.5)
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 12.5)
        ...
        # node0 should end up with 62.5 btc in block rewards plus fees, but
        # minus the 26 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), 62.5-26)
        assert_equal(self.nodes[2].getbalance(), 26)
        assert_equal(self.nodes[0].getbalance("*"), 62.5-26)
        assert_equal(self.nodes[2].getbalance("*"), 26)
        ...
        # Send 1.0 from an utxo of value 12.5 but don't specify a change output, so then
        # the change of 11.5 becomes the fee, which is greater than estimated fee of 0.0019.
        inputs = []
        outputs = {}
        for utxo in node2utxos:
            if utxo["amount"] == Decimal("12.5"):
                break
        assert_equal(utxo["amount"], Decimal("12.5"))
        ...
        assert("1150000000 > 190000" in errorString)
        ...
        assert_equal(self.nodes[2].getbalance(), 62.5)
        assert_equal(self.nodes[0].getbalance("*"), 0)
        assert_equal(self.nodes[2].getbalance("*"), 62.5)
        ...
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('52.49900000'))
        ...
        self.nodes[2].generate(1)
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('42.49900000'))
        ...
        self.nodes[2].generate(1)
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('32.49800000'))
        ...
        self.nodes[2].generate(1)
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('22.49800000'))
        ...
        self.sync_all()
        assert_equal(self.nodes[2].getbalance(), Decimal('22.49800000')) #should not be changed because tx was not broadcasted
        assert_equal(self.nodes[2].getbalance("*"), Decimal('22.49800000')) #should not be changed because tx was not broadcasted
        ...
        self.sync_all()
        txObjNotBroadcasted = self.nodes[0].gettransaction(txIdNotBroadcasted)
        assert_equal(self.nodes[2].getbalance(), Decimal('24.49800000')) #should not be
        assert_equal(self.nodes[2].getbalance("*"), Decimal('24.49800000')) #should not be
        ...
        assert_equal(self.nodes[2].getbalance(), Decimal('26.49800000')) #should not be
        assert_equal(self.nodes[2].getbalance("*"), Decimal('26.49800000')) #should not be
