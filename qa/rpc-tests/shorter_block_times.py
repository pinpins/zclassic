...
class ShorterBlockTimes(BitcoinTestFramework):
    def setup_nodes(self):
        return start_nodes(4, self.options.tmpdir, [[
            ...
            '-nuparams=821a451c:100', # Bubbles
            '-eqparams=821a451c:0:0',
            '-nuparams=211a0dde:106', # Buttercup
    ...
    def run_test(self):
        ...
        assert_equal(105, self.nodes[0].getrawtransaction(txid, 1)['expiryheight'])  # Buttercup activation - 1
        self.sync_all()
        ...
        self.sync_all()
        print "Mining last pre-Buttercup blocks"
        # Activate buttercup
        self.nodes[1].generate(1)
        self.sync_all()
        # Check that we received a pre-Buttercup mining reward
        assert_equal(12.5, Decimal(self.nodes[1].getwalletinfo()['immature_balance']))

        # After blossom activation the block reward will be halved
        print "Mining first Buttercup block"
        self.nodes[1].generate(1)
        self.sync_all()
        # Check that we received an additional Buttercup mining reward
        assert_equal(13.28125, self.nodes[1].getwalletinfo()['immature_balance'])
