...
class FinalizeBlockTest(BitcoinTestFramework):
    def setup_network(self):
        self.num_nodes = 3
        self.extra_args = [["-finalizationdelay=0", "-whitelist=127.0.0.1"],
                           ["-finalizationdelay=0", "-whitelist=127.0.0.1"], ["-whitelist=127.0.0.1"]]
        self.finalization_delay = int(0.5 * 60 * 60)
    ...
    def run_test(self):
        ...
        print("Test that invalidating a finalized block moves the finalization backward...")
        ...
        print("Check that finalization delay does not prevent auto-finalization")
