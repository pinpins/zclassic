...
class ParkedChainTest(BitcoinTestFramework):
    def setup_network(self):
        self.num_nodes = 2
        self.extra_args = [["-noparkdeepreorg", "-whitelist=127.0.0.1"], ["-maxreorgdepth=-1"]]
    ...
    def run_test(self):
        ...
        print("Test invalidate, park, unpark, reconsider...")
        node.generate(1)
        tip = node.getbestblockhash()
        node.generate(1)
        ...
        print("Test park, invalidate, reconsider, unpark")
        node.generate(1)
        tip = node.getbestblockhash()
        node.generate(1)
        bad_tip = node.getbestblockhash()
        ...
        print("Test invalidate, park, reconsider, unpark...")
        node.generate(1)
        tip = node.getbestblockhash()
        node.generate(1)
        bad_tip = node.getbestblockhash()
        ...
        print("Test park, invalidate, unpark, reconsider")
        node.generate(1)
        tip = node.getbestblockhash()
        node.generate(1)
        bad_tip = node.getbestblockhash()
        ...
        def check_reorg_protection(depth, extra_blocks):
            print("Test deep reorg parking, {} block deep".format(depth))
